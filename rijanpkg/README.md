...
19. 进入全屏前保存浮动窗口的 x/y/w/h 到 pre-fullscreen-*，退出全屏时恢复位置和尺寸。tiled 窗口不需要处理，wm/layout 会在下一个 manage 周期重新分配。
